...
dplyr::tibble(
  mod = c("26", "TC", "53"), 
  desc = search_modifiers(mod)) |>
  tidyr::unnest_wider(desc) |>
  dplyr::mutate(mod = NULL) |>
...
dplyr::tibble(
  status_code = LETTERS, 
  status_name = switch_status_name(status_code),
  status_description = switch_status_description(status_code)) |> 
  dplyr::filter(!is.na(status_name)) |>
  gt::gt(
    groupname_col = "status_code", 
    row_group_as_column   = TRUE) |>
  gt_theme_northstar(
    column_labels = FALSE,
    tab_size = 22,
    tab_weight = "bold") |> 
  gt::tab_header(
    title = gt::md("Status Codes"),
    subtitle = gt::md(stnote),
    ) |>
  gt::tab_style(
    style = gt::cell_fill(color = "yellow", alpha = 0.3),
    locations = gt::cells_body(columns = gt::everything(), rows = c(1, 12, 13))
  )
```
